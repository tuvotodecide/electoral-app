...
import { setReceiveBsData, setReceiveTokenData } from "../../../redux/action/walletAction";
import CDropdown from "../../../components/common/CDropdown";
import CListCard from "../../../components/common/CLIstCard";

export default function ReceiveTokenDetails({navigation}) {
  const colors = useSelector(state => state.theme.theme);
  const userBalances = useSelector(state => state.account.tokenBalances);
	const defaultQrData = useSelector(state => state.receiveToken);
  ...
  const onUpdateQr = async () => {
		dispatch(setReceiveTokenData(editData));
